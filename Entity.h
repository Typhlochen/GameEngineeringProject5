...
enum PlayerState { STAND, WALK };
enum AIType { WALKER, GUARD, STARE };
enum AIState { WALKING, IDLE, DYING };
...
public:
    ...
    bool      const get_kill_sound() const { return play_kill; }
    ...
    void const set_ai_state(AIState new_state)
    {
        ...
        m_animation_rows = (int)m_animations[m_ai_state].size();
    };
    ...
    void const set_texture_id(std::vector<GLuint> new_texture_ids) { m_texture_ids = new_texture_ids; }
    ...
    void const set_win(bool win) { m_win = win; }
    void const set_lose(bool lose) { m_lose = lose; }
    void const set_kill_sound(bool kill) { play_kill = kill; }
    void const set_lives(int lives) { m_lives = lives; }
